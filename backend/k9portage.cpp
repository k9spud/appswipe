    ...
    foreach(s, lines)
    {
        ...
        if(match.hasMatch())
        {
            ...
            if(filter == "=")
            {
                ...
                if(clauses.isEmpty())
                {
                    output << "Empty WHERE clause for:" << s << Qt::endl;
                    continue;
                }
                ...
                if(query.exec() == false)
                {
                    output << "FAIL:" << query.lastError().text() << Qt::endl;
                    output << "  " << query.executedQuery() << Qt::endl;
                }
            ...
            output << "Unknown mask filter type:" << filter << Qt::endl;
            output << "  " << s << Qt::endl;
            continue;
        }
        ...
        if(match.hasMatch())
        {
            ...
            output << "Unknown mask filter type (Depend Repo):" << filter << Qt::endl;
            output << "  " << s << Qt::endl;
            continue;
        }

        output << "Could not parse depend string:" << Qt::endl;
        output << "  " << s << Qt::endl;
        continue;
    }
}
...
void K9Portage::applyMasks(QSqlDatabase& db, int& currentProgress, QString atomFilter)
{
    QDir dir;
    ...
    currentProgress = 100 - repos.count();
    foreach(QString repo, repos)
    {
